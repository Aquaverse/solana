//! The `server` module hosts all the RPC server microservices.
...
impl Default for ServerConfig {
    fn default() -> Self {
        Self {
            ...
            poh_verify: true,
            require_tower: false,
            tower_storage: Arc::new(solana_core::tower_storage::NullTowerStorage::default()),
            debug_keys: None,
...
impl Default for ServerStartProgress {
    fn default() -> Self {
        Self::Initializing
    }
}
...
impl Server {
    ...
    pub fn new(
        ...
        start_progress: Arc<RwLock<ServerStartProgress>>,
        ...
        if let Some(shred_version) = config.expected_shred_version {
            if let Some(wait_for_supermajority_slot) = config.wait_for_supermajority {
                *start_progress.write().unwrap() = ServerStartProgress::CleaningBlockStore;
        ...
        *start_progress.write().unwrap() = ServerStartProgress::CleaningAccounts;
        ...
        *start_progress.write().unwrap() = ServerStartProgress::StartingServices;
        ...
        if config.dev_halt_at_slot.is_some() {
            ...
            warn!("RPC server halted");
            *start_progress.write().unwrap() = ServerStartProgress::Halted;
            std::thread::park();
        }
        ...
        *start_progress.write().unwrap() = ServerStartProgress::Running;
        Self {
            gossip_service,
...
fn post_process_restored_tower(
    restored_tower: solana_core::consensus::Result<Tower>,
        ...
        .and_then(|tower| {
            ...
            if let Some(wait_slot_for_supermajority) = config.wait_for_supermajority {
                if root_bank.slot() == wait_slot_for_supermajority {
                    ...
                    return Err(solana_core::consensus::TowerError::HardFork(wait_slot_for_supermajority));
                }
            }
            ...
            if let Some(warp_slot) = config.warp_slot {
                ...
                should_require_tower = false;
                return Err(solana_core::consensus::TowerError::HardFork(warp_slot));
            }
...
fn new_banks_from_ledger(
    ...
    *start_progress.write().unwrap() = ServerStartProgress::LoadingLedger;
...
fn wait_for_supermajority(
    ...
    *start_progress.write().unwrap() = ServerStartProgress::WaitingForSupermajority;
    for i in 1.. {
        if i % 10 == 1 {
            info!(
...
mod tests {
    ...
    fn validator_exit() {
        ...
        let start_progress = Arc::new(RwLock::new(ServerStartProgress::default()));
        ...
        let mut validators: Vec<Validator> = (0..2)
            .map(|_| {
                ...
                Validator::new(
                    ...
                    &config,
                    true, // should_check_duplicate_instance
                    Arc::new(RwLock::new(ServerStartProgress::default())),
                    SocketAddrSpace::Unspecified,
                )
            })
            .collect();
